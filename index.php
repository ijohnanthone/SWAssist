...
if ($page === 'cases') {
	...
	$cases = query($conn, "SELECT c.*, u.full_name AS student_name, cs.presenting_problem, (SELECT a.title FROM activities a WHERE a.case_id = c.id ORDER BY a.activity_date DESC, a.id DESC LIMIT 1) AS latest_activity_title, (SELECT a.activity_date FROM activities a WHERE a.case_id = c.id ORDER BY a.activity_date DESC, a.id DESC LIMIT 1) AS latest_activity_date FROM cases c LEFT JOIN users u ON u.id = c.assigned_student_id LEFT JOIN case_studies cs ON cs.case_id = c.id $where ORDER BY c.updated_at DESC", $types, $params);
	render_header('Cases'); ?><div class="page-head"><div><p class="eyebrow">Records</p><h1>Cases</h1><p class="muted">Search by case ID or permitted client identifier.</p></div><a class="button primary" href="index.php?page=case-create">+ New case</a></div><form class="toolbar"><input name="search" value="<?= e($search) ?>" placeholder="Search cases"><select name="status"><option value="">All statuses</option><?php foreach (['Draft','Active','Completed','Archived'] as $status): ?><option <?= $statusFilter === $status ? 'selected' : '' ?>><?= $status ?></option><?php endforeach; ?></select><button class="button" type="submit">Filter</button></form><div class="table-wrap"><table><thead><tr><th>Case ID</th><th>Client</th><th>Status</th><th>Assigned student</th><th>Updated</th><th>Actions</th></tr></thead><tbody><?php while ($case = $cases->fetch_assoc()): ?><tr><td><div class="case-preview-wrap"><a class="case-id-link" href="index.php?page=case&id=<?= e((string)$case['id']) ?>" data-case-preview><strong><?= e($case['case_code']) ?></strong></a><div class="case-preview" id="case-preview-<?= e((string)$case['id']) ?>" role="dialog" aria-label="Preview of case <?= e($case['case_code']) ?>"><button type="button" class="preview-close" data-preview-close aria-label="Close preview">&times;</button><p class="eyebrow">Case preview</p><h2><?= e($case['case_code']) ?></h2><dl><div><dt>Client</dt><dd><?= e($case['client_name']) ?></dd></div><div><dt>Status</dt><dd><span class="badge <?= strtolower($case['status']) ?>"><?= e($case['status']) ?></span></dd></div><div><dt>Assigned student</dt><dd><?= e($case['student_name'] ?? 'Unassigned') ?></dd></div><?php if (!empty($case['latest_activity_title'])): ?><div><dt>Latest activity</dt><dd><?= e($case['latest_activity_title']) ?><?php if (!empty($case['latest_activity_date'])): ?> · <?= e(date('M j, Y', strtotime($case['latest_activity_date']))) ?><?php endif; ?></dd></div><?php endif; ?></dl><?php if (trim((string) ($case['presenting_problem'] ?? '')) !== ''): ?><p class="preview-summary"><?= e($case['presenting_problem']) ?></p><?php else: ?><p class="muted preview-summary">No case study summary has been added yet.</p><?php endif; ?><a class="button primary" href="index.php?page=case&id=<?= e((string)$case['id']) ?>">Open full case</a></div></div></td><td><?= e($case['client_name']) ?></td><td><span class="badge <?= strtolower($case['status']) ?>"><?= e($case['status']) ?></span></td><td><?= e($case['student_name'] ?? 'Unassigned') ?></td><td><?= e(date('M j, Y', strtotime($case['updated_at']))) ?></td><td><div class="row-actions"><a class="text-link" href="index.php?page=report&amp;id=<?= e((string)$case['id']) ?>&amp;download=1" download="<?= e($case['case_code']) ?>-case-study.html" title="Download case study">Download</a><a class="text-link" href="index.php?page=case&amp;id=<?= e((string)$case['id']) ?>">Open</a></div></td></tr><?php endwhile; ?></tbody></table></div><?php render_footer(); exit;
}
